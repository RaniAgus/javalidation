package io.github.raniagus.javalidation;

import java.util.Collections;
import java.util.List;
import java.util.Optional;
import java.util.function.Function;
import java.util.function.Predicate;

public class TryExitoso<T> implements Try<T> {
  private T valor;

  protected TryExitoso(T valor) {
    this.valor = valor;
  }

  @Override
  public boolean esExitoso() {
    return true;
  }

  @Override
  public T getValor() {
    return valor;
  }

  @Override
  public List<String> getErrores() {
    return Collections.emptyList();
  }

  @Override
  public Try<T> filter(Predicate<T> predicado, String mensajeDeError) {
    return predicado.test(valor) ? this : Try.fallido(mensajeDeError);
  }

  @Override
  public <R> Try<R> map(Function<T, R> function) {
    return Try.exitoso(function.apply(valor));
  }

  @Override
  public <R> Try<R> mapCatching(Function<T, R> function, String error) {
    return Try.desde(() -> function.apply(valor), error);
  }

  @Override
  public <R> Try<R> flatMap(Function<T, Try<R>> function) {
    return function.apply(valor);
  }

  @Override
  public <R> Try<R> flatMapCatching(Function<T, Optional<R>> function, String error) {
    return mapCatching(function, error)
        .flatMap(o -> o.map(Try::exitoso).orElseGet(() -> Try.fallido(error)));
  }

  @Override
  public <R> R fold(Function<List<String>, R> error, Function<T, R> exito) {
    return exito.apply(valor);
  }
}
